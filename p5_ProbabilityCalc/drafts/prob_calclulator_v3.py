...
class Hat():
    ...
    def draw(self, num_balls):
        draft_contents = self.contents.copy()       # aux. lst to bemodified in the process
        maxrand = len(self.contents) - 1            # unpper randint limit
        drwlst = []                                 # list to be returned
        if num_balls > len(self.contents):
            drwlst = self.contents.copy()
        else:
            for i in range(num_balls):
                ix = randint(0, maxrand - i)
                drwlst.append(draft_contents.pop(ix))
        return drwlst
